...
export default function RuleActivity({ ruleId }: { ruleId: number }) {
  ...
  const terminalJobCount = jobs.length - activeJobs.length;
    ...
    <div className="space-y-6">
      <section className="rounded-2xl border border-amber-200 bg-amber-50/60 p-5 dark:border-amber-900/60 dark:bg-amber-950/20">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-amber-700 dark:text-amber-300">Inference recovery</p>
            <h3 className="mt-1 text-lg font-semibold">Recovery work for this rule</h3>
          </div>
          <span className="rounded-full bg-amber-200/70 px-2.5 py-1 text-xs font-medium text-amber-900 dark:bg-amber-900/60 dark:text-amber-100">{activeJobs.length} active · {terminalJobCount} not queued</span>
        </div>
        {jobs.length === 0 ? (
          <p className="mt-3 text-sm text-amber-800 dark:text-amber-200">No recovery jobs for this rule.</p>
        ) : (
            ...
            {jobs.map((job) => (
              <div key={job.id} className="rounded-xl border border-amber-200 bg-white/80 p-3 dark:border-amber-900/60 dark:bg-gray-900/50">
                <div className="flex flex-wrap items-start gap-3">
                  ...
                  <button type="button" onClick={() => setDryRunEmailId(job.email_id)} className="rounded-lg px-2.5 py-1.5 text-xs font-medium text-blue-600 hover:bg-blue-50 dark:text-blue-300 dark:hover:bg-blue-950/40">Dry run</button>
      ...
      {dryRunEmailId && <PipelineDryRunDialog emailId={dryRunEmailId} onClose={() => setDryRunEmailId(null)} />}
    </div>
  );
}
